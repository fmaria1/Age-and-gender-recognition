...
def upload_photo():
    try:
        global fullImage, label_flag, inter
...
def predict_image_resnet():
    ...
    for img in files:
        ...
        if predicted_gender_res == 1:
            gen_res_label = Label(person_label, text="Gender : female", font=("Arial Bold", 10))
            gen_res_label.pack(side=BOTTOM, padx=5, pady=5)
        else:
            gen_res_label = Label(person_label, text="Gender : male", font=("Arial Bold", 10))
            gen_res_label.pack(side=BOTTOM, padx=5, pady=5)
